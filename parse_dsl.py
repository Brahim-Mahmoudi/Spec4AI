...
class DSLTransformer(Transformer):
    """
    Transformer qui convertit les noeuds de l'arbre (Tree) généré par Lark
    en objets Python (DSLRule, ou chaînes de caractères, etc.)
    """
    ...
    def cond_exists_binding(self, var, context, condition):
        ...
        if context in context_map:
            ...
            # Contexte par défaut : pas de restriction de type
...
def main():
    parser_arg = argparse.ArgumentParser(
        description="Génère du code Python à partir des règles DSL."
    )
    parser_arg.add_argument("--dsl", required=True, help="Chemin vers le fichier DSL (rules.dsl)")
    parser_arg.add_argument("--output", required=True, help="Chemin vers le fichier Python généré")
    ...
    try:
        ...
        print("Erreur lors de la lecture du fichier DSL:", e)
        sys.exit(1)
    ...
    try:
        ...
        print("Erreur lors du parsing DSL:", e)
        sys.exit(1)
    ...
    try:
        ...
        print(f"Code Python généré écrit dans {args.output}")
    except Exception as e:
